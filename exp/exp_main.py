    ...
    def test(self, setting, test=0):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(test_loader):
                ...
                # Print batch_y and outputs
                # print(f"Batch {i} - batch_y:")
                # print(batch_y)
                # print(f"Batch {i} - outputs:")
                # print(outputs)
                # print(len(batch_y[0][0]))
                 
                output = outputs[:, -self.args.pred_len:, f_dim:]
                ...
                pred = output # outputs.detach().cpu().numpy()  # .squeeze()
                true = batch_y_true  # batch_y.detach().cpu().numpy()  # .squeeze()
                
                for sample in batch_y :
                    sample = test_data.inverse_transform(sample)
                    new_sample = sample[:, f_dim:]
                    stock_actuals.append(new_sample)

                for sample in outputs :
                    sample = test_data.inverse_transform(sample)
                    new_sample = sample[:, f_dim:]
                    stock_predictions.append(new_sample)   
        ...
        stock_actuals = np.array(stock_actuals)
        stock_predictions = np.array(stock_predictions)
        print(stock_actuals.shape)
        print(stock_predictions.shape)
        np.save(folder_path + 'stock_actuals.npy', stock_actuals)
        np.save(folder_path + 'stock_predictions.npy', stock_predictions)
        true = []
        pred = []
        ...
        result = '_'.join(setting.split('_')[:2])
